...
# Dropdown to select the number of EVs
st.image("ev.jpg")
num_evs = st.selectbox("How many EVs do you have?", [0, 1])
...
if num_evs == 1:
    st.info("Please enter your EV details and commute information below. The default values that are displayed are for a typical UK EV-user who commutes to work on every weekday with a Tesla Model S. We recommend setting a lower and upper bound for the State-of-Charge when charging or discharging the EV battery to extend the battery lifetime.", icon="ℹ️")
    ...
    with st.form("ev_commute_details"):
        ...
        st.markdown("Your EV usage trace is computed using the SPAGHETTI tool which is available [here](https://energyinformatics.springeropen.com/articles/10.1186/s42162-024-00314-6).")
        if submitted:
    ...
    st.write("Please proceed to the Results section if you do not have an EV.")
